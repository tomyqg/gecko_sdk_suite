...
void GRAPHICS_DrawInitScreen(GLIB_Context_t *pContext)
{
  ...
  sprintf(buff,
          "EFR32 %03luMHz %+lidBm",
          (channelConfigs[rangeTest.phy]->configs[0].baseFrequency / 1000000ul),
          (long int)(RAIL_GetTxPowerDbm(railHandle) / 10));
  GLIB_DRAW_STRING_CENTER(buff, 100, 0);

  sprintf(buff, "DMP%s", buf);
  GLIB_DRAW_STRING_CENTER(buff, 110, 0);
...
uint8_t GRAPHICS_DrawConstInfo(GLIB_Context_t *pContext, uint8_t row)
{
  ...
  if (RADIO_MODE_TX == rangeTest.radioMode) {
    ...
    if ( 0u < (channelConfigs[rangeTest.phy]->configs[0u].baseFrequency % 1000000ul)) {
      uint16_t baseFreq = (channelConfigs[rangeTest.phy]->configs[0u].baseFrequency / 1000000ul);
      sprintf(buff,
              "%u.%02uMHz",
              baseFreq,
    ...
    if (RADIO_MODE_RX == rangeTest.radioMode) {
      // Check if frequency has fractional part in MHz scale
      if (0u < channelConfigs[rangeTest.phy]->configs[0u].baseFrequency % 1000000ul) {
        uint16_t baseFreq = (uint16_t) (channelConfigs[rangeTest.phy]->configs[0u].baseFrequency / 1000000ul);
        sprintf(buff,
                "F:%uM%02u",
                baseFreq,
                (uint16_t) ((channelConfigs[rangeTest.phy]->configs[0u].baseFrequency % 1000000ul) / 10000u));
    ...
    uint16_t channelSpacing = (uint16_t) (channelConfigs[rangeTest.phy]->configs[0u].channelSpacing / 1000u);
    sprintf(buff,
            "Fch:%4ukHz  Ch#: %3u",
            channelSpacing,
            (uint8_t) rangeTest.channel);
    GLIB_drawString(pContext, buff, strlen(buff), 1u, row, false);
    row += GRAPHICS_FONT_HEIGHT;
  ...
  if (RADIO_MODE_TX == radioMode) {
    ...
    if (RANGETEST_PACKET_COUNT_INVALID == pktsRcvd) {
      ...
      if (MA < 10.0f) {
        tmpMaStr[0] = ' ';
        tmpMaStr[1] = ' ';
        sprintfFloat(&tmpMaStr[2], 4, MA, 1);
      } else if (MA < 100.0f) {
        tmpMaStr[0] = ' ';
        sprintfFloat(&tmpMaStr[1], 5, MA, 1);
      } else {
        sprintfFloat(tmpMaStr, 6, MA, 1);
      }

      if (PER < 10.0f) {
        tmpPerStr[0] = ' ';
        tmpPerStr[1] = ' ';
        sprintfFloat(&tmpPerStr[2], 4, PER, 1);
      } else if (PER < 100.0f) {
        tmpPerStr[0] = ' ';
        sprintfFloat(&tmpPerStr[1], 5, PER, 1);
      } else {
        sprintfFloat(tmpPerStr, 6, PER, 1);
      }
