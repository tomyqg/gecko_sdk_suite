...
enum {
  ...
  // Admit joins only if there is an entry in the transient key table. This
  // corresponds to the Base Device Behavior specification where a Trust Center
  // enforces all devices to join with an install code-derived link key.
...
// Networking Frames
  ...
  EZSP_JOIN_NETWORK_DIRECTLY                    = 0x3B,
  ...
  EZSP_GET_FIRST_BEACON                         = 0x3D,
  EZSP_GET_NEXT_BEACON                          = 0x04,
  EZSP_GET_NUM_STORED_BEACONS                   = 0x08,
  EZSP_CLEAR_STORED_BEACONS                     = 0x3C,
...
// Messaging Frames
  ...
  EZSP_UNICAST_CURRENT_NETWORK_KEY              = 0x50,
