                     ...
                     uint8_t sequence)
{
  ...
  *messageTag = ezspNextSequence();
  ...
  switch (type) {
    ...
    {
      status = emberAfEzspSetSourceRoute(emberGetBindingRemoteNodeId(indexOrDestination));
      if (status == EMBER_SUCCESS) {
    ...
    {
          #ifdef EMBER_AF_PLUGIN_ADDRESS_TABLE
      status = emberAfEzspSetSourceRoute(emberAfPluginAddressTableLookupNodeIdByIndex(indexOrDestination));
      if (status == EMBER_SUCCESS) {
        status = ezspSendUnicast(type,
                                 indexOrDestination,
                                 apsFrame,
