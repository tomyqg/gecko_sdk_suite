...
void tryToJoinNetwork()
{
  ...
  // If the retrieved beacon channel is not set in the mask, get the next
  // beacon whose channel is set in the channel mask
  while ((status == EMBER_SUCCESS)
         && !(CURRENT_CHANNEL_MASK & BIT32(BEACON_ITERATOR.beacon.channel))) {
...
// This function is called after all channels have been scanned
HIDDEN void scanResultsHandler(EmberAfPluginScanDispatchScanResults *results)
{
...
EmberStatus emberAfPluginNetworkSteeringStart(void)
{
  ...
  if (emAfProIsCurrentNetwork()
    ...
    if (emberAfNetworkState() == EMBER_NO_NETWORK) {
      ...
      scanData.channelMask = emAfPluginNetworkSteeringPrimaryChannelMask
      ...
      emberAfCorePrintln("%s: issuing scan on primary and secondary channels "
                         "(combined channel mask 0x%4X)",
                         PLUGIN_NAME,
