                                                      ...
                                                      void *data)
{
  ...
  switch (packetType) {
    ...
    default:
      emberAfCorePrintln("Error: unsupported incoming EmberZigbeePacketType %X.", packetType);
      break;
  }
  return act;
  #else
                                                      ...
                                                      void *data)
{
  ...
  switch (packetType) {
    ...
    case EMBER_ZIGBEE_PACKET_TYPE_MAC_COMMAND: {
      ...
      emberAfCorePrint("mac hdr:tx %d bytes [", macHeaderLength);
      emberAfCorePrintBuffer(packetData, macHeaderLength, true); // spaces?
      emberAfCorePrintln("]");
      emberAfCorePrint("mac pay:tx %d bytes [", payloadDataLength);
      emberAfCorePrintBuffer(packetData + macHeaderLength, payloadDataLength, true);
      emberAfCorePrintln("]");
